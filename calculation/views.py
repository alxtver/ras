...
def base(request):
    ...
    if request.method == 'POST':
        ...
        base_dict = {value: {'number': base_dict[value]['number'],
                             'price': base_dict[value]['price'],
                             'summ': base_dict[value]['summ'],
                             'weight': base_dict[value]['weight']} for value in base_dict if int(base_dict[value]['number']) != 0}
        # расчет суммы цен и веса
        for value in base_dict:
            base_dict[value]['summ'] = round(
                float(base_dict[value]['number']) * float(base_dict[value]['price']), 2)
            base_dict[value]['weight'] = round(
                float(base_dict[value]['number']) * float(base_dict[value]['weight']), 1)
        # сортировка вывода
...
def catalog(request):
    ...
    return render(request, 'alldb.html', {'maches': maches})
...
def search(request):
    ...
    if 'q' in request.GET:
        ...
        if not q:
            ...
            return render_to_response('search.html', {'matches': matches, 'query': q, 'notfound': notfound})
...
def excel(request):
    ...
    if list_post:
        ...
        for i in list_post:
            response_record = {}
            response_record['pname'] = i.split('//')[0]
            response_record['pnumber'] = i.split('//')[1]
            response_record['pprice'] = i.split('//')[2]
            response_record['psumm'] = i.split('//')[3]
            response_record['pweight'] = i.split('//')[4]
            response_data.append(response_record)
        final_response['product'] = response_data
        request.session['product'] = final_response
